...
export function TableOfContents({
  ...
  return (
    <nav className="sticky top-24 max-h-[calc(100vh-6rem)] overflow-auto">
      {headings.length > 0 && (
        <>
          <h4 className="mb-4 text-sm text-white/80">On this page</h4>
          <ul className="border-gray-lightest border-l text-sm">
            {headings.map((heading) => {
              const isVisible = visibleIds.has(heading.id)
              return (
                <li key={heading.id} className="relative">
                  {isVisible && (
                    <span className="bg-primary absolute left-0 top-0 h-full w-0.5 -translate-x-px" />
                  )}
                  <a
                    href={`#${heading.id}`}
                    className={cx(
                      'block py-1.5 pl-4 transition-colors hover:text-white',
      ...
      <div className="border-gray-lightest mt-6 flex flex-col gap-2 py-4 border-t">
        <Link
          href={`https://github.com/0xa3k5/web3icons/edit/main/${githubPath}`}
          className="flex items-center gap-2 px-4 font-mono text-sm text-white/60 transition-colors hover:text-white"
          target="_blank"
