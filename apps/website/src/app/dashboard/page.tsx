  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border border-gray-lightest p-6">
        ...
        {/* Create New Key Form */}
        {isCreating && (
          <div className="mb-6 border border-gray-lightest p-4 bg-gray-dark">
            <div className="space-y-4">
              <input
                ...
                className="w-full bg-gray px-3 py-2 text-sm border border-gray-lightest focus:outline-none focus:border-white/40"
              />
        ...
        {newApiKey && (
          <div className="mb-6 border border-primary bg-primary/10 p-4">
            <p className="text-sm mb-2">
              <span className="text-white/60">your new api key (save it now, you won't see it again):</span>
            </p>
            <div className="flex items-center gap-2">
              <code className="bg-gray-dark px-3 py-2 text-xs border border-gray-lightest flex-1">
                {newApiKey}
              </code>
              <button
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            apiKeys.map((key) => (
              <div key={key.id} className="border border-gray-lightest p-4 flex items-center justify-between">
                <div>
                  <p className="font-mono">{key.name}</p>
                  <p className="text-xs text-white/40">
                    {key.key_prefix}...{key.last_4} • created {new Date(key.created_at).toLocaleDateString()}
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <pre className="mt-2 bg-gray-dark border border-gray-lightest p-3 text-xs overflow-x-auto">
  ...
  "https://web3icons-production.up.railway.app/v1/icons/tokens/ETH"`}
      ...
      <div className="border border-gray-lightest p-6">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-mono">{usage.requests}</p>
            <p className="text-xs text-white/40">requests</p>
          </div>
          <div>
            <p className="text-2xl font-mono">{usage.remaining}</p>
            <p className="text-xs text-white/40">remaining</p>
          </div>
          <div>
            <p className="text-2xl font-mono">{usage.percentUsed}%</p>
