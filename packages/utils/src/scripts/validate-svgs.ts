...
const isUppercase = (filename: string): boolean => filename === filename.toUpperCase()

const isKebabCase = (filename: string): boolean => /^[a-z0-9]+(-[a-z0-9]+)*\.svg$/.test(filename)
...
const validateSvg = (filePath: string, isToken: boolean): void => {
    ...
    // check size
    const hasCorrectDimensions = svgContent.includes('width="24"') && svgContent.includes('height="24"');
    ...
    if (isToken && !isUppercase(fileName)) {
        throw new Error(`Invalid file name for token ${filePath}. Expected uppercase.`);
    }
    if (!isToken && !isKebabCase(fileName)) {
        throw new Error(`Invalid file name for network ${filePath}. Expected kebab-case.`);
    }
};
