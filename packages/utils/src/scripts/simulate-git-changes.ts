...
const getNewIcons = (path: string) => {
  return executeCommand(`git ls-files --others --exclude-standard -- '*.svg' | grep '${path}' | xargs basename -a | sed 's/.svg//' | sort -u | tr '\\n' ',' | sed 's/,$//'`);
};

const formatIconsForMessage = (icons: string) => {
  return icons.split(',').map(icon => `\`${icon.trim()}\``).join(', ');
};
