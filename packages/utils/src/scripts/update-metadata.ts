...
const appendToNetworksJson = async (network: INetworkMetadata): Promise<void> => {
  const jsonPath = path.resolve(process.cwd(), 'packages/core/src/metadata/networks.json');
  ...
  const existingNetworkIndex = existingMetadata.findIndex((n) => n.id === network.id);
  ...
  const formatted = await prettier.format(JSON.stringify(existingMetadata), { parser: 'json' });
  fs.writeFileSync(jsonPath, formatted);
};

const appendToTokensJson = async (coin: ITokenMetadata): Promise<void> => {
  const jsonPath = path.resolve(process.cwd(), 'packages/core/src/metadata/tokens.json')
  ...
  fs.writeFileSync(jsonPath, await prettier.format(JSON.stringify(existingMetadata), { parser: 'json' }));
};

const processSVGFile = async (file: string, type: 'token' | 'network'): Promise<void> => {
  const fileName = type === 'token' ? path.basename(file, '.svg').toUpperCase() : path.basename(file, '.svg');
  console.log(fileName)
  const geckoCoins = JSON.parse(fs.readFileSync(path.join(__dirname, './gecko/gecko-coins.json'), 'utf8'));
  const geckoNetworks = JSON.parse(fs.readFileSync(path.join(__dirname, './gecko/gecko-networks.json'), 'utf8'));

  if (type === 'token') {
    const foundCoin = geckoCoins.find((coin: any) => coin.symbol.toUpperCase() === fileName);
    if (foundCoin) {
    ...
    const foundNetwork = geckoNetworks.find((network: any) => 
...
const main = async (): Promise<void> => {
  const newSVGFiles: { file: string; type: 'token' | 'network' }[] = process.argv.slice(2).map(file => ({
    file, 
