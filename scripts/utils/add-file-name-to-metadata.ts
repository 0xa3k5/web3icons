...
export const addFileNameToMetadata = () => {
  ...
  const tokenMetadata: ITokenMetadata[] = JSON.parse(
  ...
  const networks = matchMetadataWithFiles(
    networkFileNames,
    networkMetadata,
    'network',
  )
  const wallets = matchMetadataWithFiles(
    walletFileNames,
    walletMetadata,
    'wallet',
  )
  const exchanges = matchMetadataWithFiles(
    exchangeFileNames,
    exchangeMetadata,
    'exchange',
  )
  ...
  fs.writeFileSync(
    NETWORKS_METADATA_PATH,
    JSON.stringify(networks.matched, null, 2),
  )
  fs.writeFileSync(
    WALLETS_METADATA_PATH,
    JSON.stringify(wallets.matched, null, 2),
  )
  fs.writeFileSync(
    EXCHANGES_METADATA_PATH,
    JSON.stringify(exchanges.matched, null, 2),
  )
  fs.writeFileSync(
    TOKENS_METADATA_PATH,
    JSON.stringify(tokens.matched, null, 2),
  )
