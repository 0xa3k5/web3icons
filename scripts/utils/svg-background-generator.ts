...
function extractGradientAndColor(svgContent: string): {
  color: string
  defs: string | null
} {
  ...
  const dominantColor =
    Object.values(colorAreas).length > 0
      ? (Object.values(colorAreas).sort((a, b) => b.area - a.area)[0]?.color ??
        '#000000')
      : gradientDefs
...
function createBackgroundVariant(
  svg: string,
  color: string,
  gradientDefs: string | null,
): string {
...
export function generateBackgroundVariants(
  ...
  brandedFiles.forEach((file) => {
    ...
    const monoContent = fs.existsSync(monoPath)
      ? fs.readFileSync(monoPath, 'utf-8')
      : null

    const { color: dominantColor, defs: gradientDefs } =
      extractGradientAndColor(brandedContent)
