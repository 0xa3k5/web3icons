...
export const findRawData = (name: string, type: TType): TRaw[] | undefined => {
  if (type === 'token') {
    ...
    const geckoNetworks = findByFileName(
      type,
      name,
      _geckoNetworks as INetworkRaw[],
    )
    const customNetworks = findByFileName(
      type,
      name,
      _customNetworks as INetworkRaw[],
    )
    ...
    const customWallets = findByFileName(
      type,
      name,
      walletsMetadata as IWalletRaw[],
    )
    ...
    const customExchanges = findByFileName(
      type,
      name,
      exchangesMetadata as IExchangeRaw[],
    )
