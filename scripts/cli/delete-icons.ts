...
const deleteVariant = async (
  ...
  console.log(
    chalk.blue(`Removed ${deletedVariant} variant from ${metadata.id}`),
  )
}
...
export const handleDeletedIcons = async (): Promise<void> => {
  ...
  for (const filePath of deletedIconPaths) {
    ...
    if (!existingMetadata) {
      console.log(
        chalk.yellow(`No metadata found for ${fileName}, safe to delete`),
      )
      continue
    }
    ...
    if (existingMetadata.variants.length > 1) {
      ...
      const remainingVariants = existingMetadata.variants.filter(
        (v) => v !== variant,
      )
